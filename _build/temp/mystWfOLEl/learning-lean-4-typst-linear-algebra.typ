...
A scratch chapter for linear-algebra notes — enough structure to flesh out later, and enough variety (numbered equations, a table, admonitions, and interleaved Lean) to show the book template in action. The runnable code lives in #link("https://live.lean-lang.org/\#url=https%3A%2F%2Fraw.githubusercontent.com%2Fjoshuawkelly%2FMathematics-Notebook%2Fmain%2FLearningLean%2FLinearAlgebra.lean")[`LearningLean/LinearAlgebra.lean`].
...
In Lean, a vector is a function `Fin n → ℝ`, and `![…]` is notation for one. The dot product and the matrix operations below follow the spellings used in Mathlib #cite(<mathlib2020>):#footnote[Mathlib writes the dot product as `⬝ᵥ` (`\cdotv`) rather than reusing `·`, to keep it distinct from scalar multiplication.]
