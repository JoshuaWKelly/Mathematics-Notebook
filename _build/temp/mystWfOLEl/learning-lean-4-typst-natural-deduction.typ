...
The Lean proofs for this chapter live in #link("https://live.lean-lang.org/\#url=https%3A%2F%2Fraw.githubusercontent.com%2Fjoshuawkelly%2FMathematics-Notebook%2Fmain%2FLearningLean%2FNaturalDeduction.lean")[`LearningLean/NaturalDeduction.lean`], inside the `Exercises` namespace.
...
#figure(
  image("files/fitch-59408cd6bd473bfcf5765af9a8504464.svg", width: 40%),
  kind: "figure",
  supplement: [Figure],
) <fig-fitch-5-5>
...
In Lean this is theorem `T55`, where `¬P` is definitionally `P → False`,#footnote[This is the standard definition in Lean's core library: `Not p` unfolds to `p → False`, so a proof of `¬P` is exactly a function turning a proof of `P` into a proof of `False`.] so the whole proof is just function composition:
